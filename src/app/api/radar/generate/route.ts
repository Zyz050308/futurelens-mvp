...
优先级顺序（从高到低）：
1. userStateProfile.state（用户状态）
2. userStateProfile.strategyFocus（当前状态下真正应该关注的策略方向）
3. userStateProfile.actionBias（行动生成时应该优先选择的动作类型）
4. userStateProfile.forbiddenBias（行动生成时应该避免的动作类型）
5. 用户 profile 中的时间 / 风险 / 焦虑 / 能力
6. changeSignals（今日变化信号，作为现实依据）
7. domain / 职业背景

也就是说：用户状态 > 目标焦虑 > 时间风险能力 > 今日变化 > 职业标签
...
【V6.1 CoreInsight 强制要求】

CoreInsight 必须满足：
1. 反直觉 - 不能是常识（如"学习AI"、"提升能力"、"保持竞争力"）
2. 必须引用用户真实情况（时间、风险、目标等）
3. 必须指出真正问题
...
示例 - 设计师：
你以为：AI正在抢走设计师工作
实际上：AI正在压缩客户为设计执行付费的意愿（客户不愿为执行付高价，但愿意为策略付更高价）

示例 - 财务：
你以为：AI会替代你
实际上：企业未来不会增加基础财务岗位（岗位数量不增反降，靠内部晋升很难）

示例 - 建筑：
你以为：考证能解决问题
实际上：你缺的不是证书，而是从执行者到管理者的迁移能力（证书只是门槛，不是晋升保证）

示例 - 雅思：
你以为：多刷题就能考过
实际上：你缺的不是练习量，而是有效的反馈机制（低效重复100小时不如高效反馈10小时）

禁止：
- 学习AI、持续输出、提升能力、关注趋势、建立个人品牌、拥抱变化、提高竞争力
- 空泛的行业趋势描述
- 用户本来就知道的常识

必须结合用户填写的具体信息：
- 如果用户填写"每周时间5小时以下"，要指出时间限制是真正约束
- 如果用户填写"风险偏好稳妥"，要指出安稳策略本身就是风险
- 如果用户填写"想赚钱但没时间"，要指出时间和金钱是置换关系
...
- coreInsight 是 V6.1 必填项！必须包含"你以为"和"实际上"两个部分，且必须引用用户填写的具体信息（时间/风险/目标），禁止使用"学习AI"、"提升能力"等常识
